...
/**
 * Logs for a redirection-host
 *
 * /api/nginx/redirection-hosts/123/logs
 */
router
	.route("/:host_id/logs")
	.options((_, res) => {
	...
	/**
	 * GET /api/nginx/redirection-hosts/123/logs
	 */
	.get(async (req, res, next) => {
		try {
			const data = await validator(
			...
			const result = await internalRedirectionHost.getLogs(res.locals.access, {
				id: Number.parseInt(data.host_id, 10),
			});
			res.status(200).send(result);
		} catch (err) {
