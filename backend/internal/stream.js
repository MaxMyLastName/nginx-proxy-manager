...
const internalStream = {
	...
	getLogs: async (access, data) => {
		await access.can("streams:logs", data.id);
		await internalStream.get(access, { id: data.id });
		return {
			access: tailFile(`/data/logs/stream-${data.id}_access.log`),
			error: tailFile(`/data/logs/stream-${data.id}_error.log`),
		};
	},
};
