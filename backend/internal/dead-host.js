...
const internalDeadHost = {
	...
	getLogs: async (access, data) => {
		await access.can("dead_hosts:logs", data.id);
		await internalDeadHost.get(access, { id: data.id });
		return {
			access: tailFile(`/data/logs/dead-host-${data.id}_access.log`),
			error: tailFile(`/data/logs/dead-host-${data.id}_error.log`),
		};
	},
};
