...
import { useRedirectionHostLogs } from "src/hooks";
import { T } from "src/locale";

const LIVE_TAIL_INTERVAL_MS = 3000;

const showRedirectionHostLogsModal = (hostId: number, domainNames: string[]) => {
	EasyModal.show(RedirectionHostLogsModal, { hostId, domainNames });
};
...
const RedirectionHostLogsModal = EasyModal.create(({ hostId, domainNames, visible, remove }: Props) => {
	const [isLiveTailing, setIsLiveTailing] = useState(false);
	const { data, isLoading, error } = useRedirectionHostLogs(hostId, {
...
export { showRedirectionHostLogsModal };
