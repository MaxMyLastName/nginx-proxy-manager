...
import { useStreamLogs } from "src/hooks";
import { T } from "src/locale";

const LIVE_TAIL_INTERVAL_MS = 3000;

const showStreamLogsModal = (streamId: number, label: string) => {
	EasyModal.show(StreamLogsModal, { streamId, label });
};
...
const StreamLogsModal = EasyModal.create(({ streamId, label, visible, remove }: Props) => {
	const [isLiveTailing, setIsLiveTailing] = useState(false);
	const { data, isLoading, error } = useStreamLogs(streamId, {
