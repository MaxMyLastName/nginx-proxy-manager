...
const showProxyHostLogsModal = (hostId: number, domainNames: string[]) => {
	EasyModal.show(ProxyHostLogsModal, { hostId, domainNames });
};
...
const ProxyHostLogsModal = EasyModal.create(({ hostId, domainNames, visible, remove }: Props) => {
	const [isLiveTailing, setIsLiveTailing] = useState(false);
	const { data, isLoading, error } = useProxyHostLogs(hostId, {
